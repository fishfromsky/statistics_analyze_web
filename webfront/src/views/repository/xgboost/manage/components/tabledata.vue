...
<script>
import { getxgboostfilelist, getxgboostrpoject, getexcelinfo, startxgboost, addxgboostproject, amendxgboostproject } from "@/api/model";
export default {
  data() {
    return {
      tableshow: true,
  ...
  methods: {
    ...
    AmendDataConfirm:function(){
      let that = this
      amendxgboostproject(this.form).then(res=>{
        if (res.code === 20000){
          this.$message({
            type: 'success',
            message: '修改成功'
    ...
    addDataConfirm:function(){
      ...
      if (this.add_form.project_id === ''){
        ...
        let that = this
        addxgboostproject(this.add_form).then(res=>{
          if (res.code === 20000){
            this.$message({
              type: 'success',
              message: '添加成功'
