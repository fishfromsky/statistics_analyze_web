<template>
  ...
  <div class="app-container">
    <div class="filter-container">
      ...
      <el-input v-model="listQuery.name" placeholder="参数名称" style="width: 100px;" class="filter-item" @keyup.enter.native="handleFilter" />
    ...
    >
      ...
      <el-table-column label="参数名称" width="300.0px" align="center">
        <template slot-scope="{row}">
          <span>{{ row.name }}</span>
        </template>
      </el-table-column>
      <el-table-column label="参数值" width="300.0px" align="center">
        <template slot-scope="{row}">
          <span>{{ row.value }}</span>
        </template>
      </el-table-column>
      <el-table-column label="单位" width="150.0px" align="center">
        <template slot-scope="{row}">
          <span>{{ row.unit }}</span>
        </template>
      </el-table-column>
      <el-table-column label="备注" width="300.0px" align="center">
        <template slot-scope="{row}">
          <span>{{ row.note }}</span>
        </template>
      </el-table-column>
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form ref="dataForm" :rules="rules" :model="temp" label-position="left" label-width="150px" style="width: 500px; margin-left:100px;">
        ...
        <el-form-item label="参数名称">
          <el-input v-model="temp.name" type="忽略" placeholder="参数名称" />
        </el-form-item>
        <el-form-item label="参数值">
          <el-input v-model="temp.value" type="number" placeholder="输入数字" />
        </el-form-item>
        <el-form-item label="单位">
          <el-input v-model="temp.unit" type="忽略" placeholder="单位" />
        </el-form-item>
        <el-form-item label="备注">
          <el-input v-model="temp.note" type="忽略" placeholder="备注" />
        </el-form-item>
...
import { fetchList, fetchPv, createpmedianbs, updatepmedianbs, deletepmedianbs, downloadpmedianbs, uploadpmedianbs, clearpmedianbs } from '@/api/app01/pmedianbs'
...
export default {
  ...
  methods: {
    ...
    createData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          createpmedianbs(this.temp).then(() => {
    ...
    handleDownload() {
      this.listLoading = true
      downloadpmedianbs(this.listQuery).then(response => {
        // response = response.json();
        this.list = response.data.items
        // console.log('data.................', data)
        const tHeader = ['项目编号', '参数名称', '参数值', '单位', '备注']
        const filterVal = ['project_id', 'name', 'value', 'unit', 'note']
    ...
    handleClear() {
      this.listLoading = true
      this.dialogFormVisibleclear = false
      clearpmedianbs(this.listQuery).then(() => {
        this.$notify({
          title: 'Success',
          message: '清空成功',
          type: 'success',
    ...
    handleUpload() {
      ...
      uploadpmedianbs(this.tableData).then(() => {
        this.$notify({
          title: 'Success',
          message: '上传成功',
          type: 'success',
