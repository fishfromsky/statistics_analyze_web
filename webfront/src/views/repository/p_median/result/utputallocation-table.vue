<template>
  ...
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="项目编号" width="300.0px" align="center">
...
import { fetchList, fetchPv, createutputallocation, updateutputallocation, deleteutputallocation, downloadutputallocation, uploadutputallocation, clearutputallocation } from '@/api/app01/utputallocation'
...
export default {
  ...
  methods: {
    ...
    createData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          createutputallocation(this.temp).then(() => {
    ...
    updateData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          updateutputallocation(tempData).then(() => {
    ...
    handleDelete(row, index) {
      ...
      console.log(row.id)
      deleteutputallocation(row.id).then(() => {
        this.$notify({
          title: 'Success',
          message: 'Update Successfully',
          type: 'success',
    ...
    handleDownload() {
      this.listLoading = true
      downloadutputallocation(this.listQuery).then(response => {
        // response = response.json();
        this.list = response.data.items
        // console.log('data.................', data)
        const tHeader = ['项目编号', '集散场', '中转站']
        const filterVal = ['project_id', 'ts', 'rrc']
    ...
    handleClear() {
      this.listLoading = true
      this.dialogFormVisibleclear = false
      clearutputallocation(this.listQuery).then(() => {
        this.$notify({
          title: 'Success',
          message: '清空成功',
          type: 'success',
    ...
    handleUpload() {
      ...
      uploadutputallocation(this.tableData).then(() => {
        this.$notify({
          title: 'Success',
          message: '上传成功',
          type: 'success',
