...
<script>
import {
  getgreyrelationmodelresult,
  getgreyrelationresult,
...
export default {
  ...
  methods: {
    ...
    initTable: function (project_id) {
      ...
      getgreyrelationmodelresult(data).then((res) => {
        if (res.code === 20000) {
    ...
    Visualization(val) {
      ...
      getgreyrelationresult(data).then((res) => {
        if (res.code === 20000) {
            let data = res.data
            this.graph_data.label = res.label
            for (let i=0; i<data.length; i++){
                for (let j=0; j<data[i].length; j++){
                    this.graph_data.result.push([i, j, data[i][j].toFixed(2)])
                }
            }
