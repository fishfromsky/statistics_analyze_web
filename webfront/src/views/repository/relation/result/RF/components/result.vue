...
<script>
import { getrelationrf } from '@/api/model'
    ...
    methods:{
        showChart:function(id){
            ...
            for (let i=0; i<chart_data.length; i++){
                if (chart_data[i].sort === id){
                    ...
                    this.graph_data.value.push(chart_data[i].value)
                }
            }
        },
        ...
        download:function(){
            import('@/vendor/Export2Excel').then(excel => {
                const tHeader = ['Label', 'Value', 'Time', 'Sort']
                const filterVal = ['label', 'value', 'time', 'sort']
