...
var _ShadowMap = require("claygl/src/prePass/ShadowMap");
...
var _notifier = require("claygl/src/core/mixin/notifier");

var _EffectCompositor = require("../effect/EffectCompositor");

var _TemporalSuperSampling = require("../effect/TemporalSuperSampling");

var _halton = require("../effect/halton");
...
function ViewGL(projection) {
  ...
  this._compositor = new _EffectCompositor.default();
  this._temporalSS = new _TemporalSuperSampling.default();
  this._shadowMapPass = new _ShadowMap.default();
  var pcfKernels = [];
  var off = 0;
  ...
  for (var i = 0; i < 30; i++) {
    ...
    for (var k = 0; k < 6; k++) {
      pcfKernel.push((0, _halton.default)(off, 2) * 4.0 - 2.0);
      pcfKernel.push((0, _halton.default)(off, 3) * 4.0 - 2.0);
      off++;
    }
...
ViewGL.prototype.setProjection = function (projection) {
  ...
  if (projection === 'perspective') {
    if (!(this.camera instanceof _Perspective.default)) {
      this.camera = new _Perspective.default();
    ...
    if (!(this.camera instanceof _Orthographic.default)) {
      this.camera = new _Orthographic.default();
...
ViewGL.prototype._updateTransparent = function (renderer, scene, camera, frame) {
  ...
  var invWorldTransform = new _Matrix.default();
  ...
  for (var i = 0; i < transparentList.length; i++) {
    ...
    _Matrix.default.invert(invWorldTransform, renderable.worldTransform);

    _Vector.default.transformMat4(v3, cameraWorldPosition, invWorldTransform);
...
Object.assign(ViewGL.prototype, _notifier.default);
var _default = ViewGL;
