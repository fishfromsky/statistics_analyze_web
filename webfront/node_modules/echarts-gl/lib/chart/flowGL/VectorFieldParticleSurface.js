...
var _vectorFieldParticleGlsl = require("./vectorFieldParticle.glsl.js");

// import TemporalSS from '../../effect/TemporalSuperSampling';
_Shader.default['import'](_vectorFieldParticleGlsl.default);
...
var VectorFieldParticleSurface = function () {
  ...
  this.vectorFieldTexture = new _Texture2D.default({
    type: _Texture.default.FLOAT,
...
VectorFieldParticleSurface.prototype = {
  ...
  init: function () {
    var parameters = {
      type: _Texture.default.FLOAT,
      minFilter: _Texture.default.NEAREST,
      magFilter: _Texture.default.NEAREST,
      useMipmap: false
    };
    this._spawnTexture = new _Texture2D.default(parameters);
    this._particleTexture0 = new _Texture2D.default(parameters);
    this._particleTexture1 = new _Texture2D.default(parameters);
    this._frameBuffer = new _FrameBuffer.default({
      depthBuffer: false
    });
    this._particlePass = new _Pass.default({
      fragment: _Shader.default.source('ecgl.vfParticle.particle.fragment')
    });
    ...
    this._downsamplePass = new _Pass.default({
      fragment: _Shader.default.source('clay.compositor.downsample')
    });
    var particlePointsMesh = new _Mesh.default({
      // Render after last frame full quad
      renderOrder: 10,
      material: new _Material.default({
        shader: new _Shader.default(_Shader.default.source('ecgl.vfParticle.renderPoints.vertex'), _Shader.default.source('ecgl.vfParticle.renderPoints.fragment'))
      }),
      mode: _Mesh.default.POINTS,
      geometry: new _Geometry.default({
        dynamic: true,
        mainAttribute: 'texcoord0'
      })
    });
    var particleLinesMesh = new _Mesh.default({
      // Render after last frame full quad
      renderOrder: 10,
      material: new _Material.default({
        shader: new _Shader.default(_Shader.default.source('ecgl.vfParticle.renderLines.vertex'), _Shader.default.source('ecgl.vfParticle.renderLines.fragment'))
      }),
      geometry: new _Line2D.default(),
      culling: false
    });
    var lastFrameFullQuad = new _Mesh.default({
      material: new _Material.default({
        shader: new _Shader.default(_Shader.default.source('ecgl.color.vertex'), _Shader.default.source('ecgl.color.fragment')) // DO NOT BLEND Blend will multiply alpha
        // transparent: true

      }),
      geometry: new _Plane.default()
    });
    ...
    this._camera = new _Orthographic.default();
    this._thisFrameTexture = new _Texture2D.default();
    this._lastFrameTexture = new _Texture2D.default();
  },
  ...
  setParticleSize: function (size) {
    ...
    if (!this._spriteTexture) {
      this._spriteTexture = new _Texture2D.default();
    }
  ...
  _updateDownsampleTextures: function (renderer, api) {
    ...
    for (var i = 0; i < upScale; i++) {
      downsampleTextures[i] = downsampleTextures[i] || new _Texture2D.default();
