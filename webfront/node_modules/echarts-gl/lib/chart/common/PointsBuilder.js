...
var _sprite = require("../../util/sprite");

var _PointsMesh = require("./PointsMesh");
...
function PointsBuilder(is2D, api) {
  this.rootNode = new _graphicGL.default.Node();
  /**
   * @type {boolean}
   */

  this.is2D = is2D;
  this._labelsBuilder = new _LabelsBuilder.default(256, 256, api); // Give a large render order.
...
PointsBuilder.prototype = {
  ...
  update: function (seriesModel, ecModel, api, start, end) {
    ...
    if (!this._mesh) {
      ...
      this._mesh = new _PointsMesh.default({
        // Render after axes
        renderOrder: 10,
        // FIXME
    ...
    if (largeMode) {
      ...
      _graphicGL.default.parseColor(color, rgbaArr);
    ...
    for (var i = 0; i < end - start; i++) {
      ...
      if (!largeMode) {
        var color = (0, _visual.getItemVisualColor)(data, i);
        var opacity = (0, _visual.getItemVisualOpacity)(data, i);
  ...
  _updateSymbolSprite: function (seriesModel, itemStyle, symbolInfo, dpr) {
    ...
    if (this._symbolType !== symbolInfo.type || !isSymbolSizeSame(this._symbolSize, symbolSize) || this._lineWidth !== itemStyle.lineWidth) {
      _sprite.default.createSymbolSprite(symbolInfo.type, symbolSize, {
        fill: '#fff',
      ...
      _sprite.default.createSDFFromCanvas(this._spriteImageCanvas, Math.min(this._spriteImageCanvas.width, 32), SDF_RANGE, this._mesh.material.get('sprite').image);
  ...
  _updateMaterial: function (seriesModel, itemStyle) {
    var blendFunc = seriesModel.get('blendMode') === 'lighter' ? _graphicGL.default.additiveBlend : null;
    var material = this._mesh.material;
    material.blend = blendFunc;
    material.set('lineWidth', itemStyle.lineWidth / SDF_RANGE);

    var strokeColor = _graphicGL.default.parseColor(itemStyle.stroke);
  ...
  _updateAnimation: function (seriesModel) {
    _graphicGL.default.updateVertexAnimation([['prevPosition', 'position'], ['prevSize', 'size']], this._prevMesh, this._mesh, seriesModel);
  },
  ...
  updateView: function (camera) {
    ...
    var worldViewProjection = new _Matrix.default();

    _Matrix.default.mul(worldViewProjection, camera.viewMatrix, this._mesh.worldTransform);

    _Matrix.default.mul(worldViewProjection, camera.projectionMatrix, worldViewProjection);
  ...
  _getSymbolInfo: function (seriesModel, start, end) {
    if (seriesModel.get('large')) {
      var symbolSize = _retrieve.default.firstNotNull(seriesModel.get('symbolSize'), 1);
