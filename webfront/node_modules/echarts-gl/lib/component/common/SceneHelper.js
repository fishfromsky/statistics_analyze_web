...
var _Skybox = require("claygl/src/plugin/Skybox");
...
SceneHelper.prototype = {
  ...
  initLight: function (rootNode) {
    ...
    this.mainLight = new _graphicGL.default.DirectionalLight({
      shadowBias: 0.005
    });
    /**
    ...
    this.ambientLight = new _graphicGL.default.AmbientLight();
  ...
  updateLight: function (componentModel) {
    ...
    mainLight.color = _graphicGL.default.parseColor(mainLightModel.get('color')).slice(0, 3);
    ambientLight.color = _graphicGL.default.parseColor(ambientLightModel.get('color')).slice(0, 3);
    var alpha = mainLightModel.get('alpha') || 0;
    var beta = mainLightModel.get('beta') || 0;
    mainLight.position.setArray(_graphicGL.default.directionFromAlphaBeta(alpha, beta));
    mainLight.lookAt(_graphicGL.default.Vector3.ZERO);
    mainLight.castShadow = mainLightModel.get('shadow');
    mainLight.shadowResolution = _graphicGL.default.getShadowResolution(mainLightModel.get('shadowQuality'));
  },
  updateAmbientCubemap: function (renderer, componentModel, api) {
    ...
    if (textureUrl) {
      ...
      if (!lights) {
        var self = this;
        lights = this._cubemapLightsCache[textureUrl] = _graphicGL.default.createAmbientCubemap(ambientCubemapModel.option, renderer, api, function () {
  ...
  updateSkybox: function (renderer, componentModel, api) {
    ...
    function getSkybox() {
      self._skybox = self._skybox || new _Skybox.default();
      return self._skybox;
    }
    ...
    if (environmentUrl && environmentUrl !== 'none') {
      if (environmentUrl === 'auto') {
        ...
        if (this._currentCubemapLights) {
          ...
          var texture = new _graphicGL.default.Texture2D({
            anisotropic: 8,
            flipY: false
          });
          ...
          var texture = _graphicGL.default.loadTexture(environmentUrl, api, {
            anisotropic: 8,
            flipY: false
          });
