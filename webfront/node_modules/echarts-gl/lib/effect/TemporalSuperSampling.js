...
function TemporalSuperSampling(frames) {
  ...
  for (var i = 0; i < 30; i++) {
    haltonSequence.push([(0, _halton.default)(i, 2), (0, _halton.default)(i, 3)]);
  }

  this._haltonSequence = haltonSequence;
  this._frame = 0;
  this._sourceTex = new _Texture2D.default();
  this._sourceFb = new _FrameBuffer.default();

  this._sourceFb.attach(this._sourceTex); // Frame texture before temporal supersampling


  this._prevFrameTex = new _Texture2D.default();
  this._outputTex = new _Texture2D.default();
  var blendPass = this._blendPass = new _Pass.default({
  ...
  this._blendFb = new _FrameBuffer.default({
    depthBuffer: false
  });
  this._outputPass = new _Pass.default({
...
TemporalSuperSampling.prototype = {
  ...
  jitterProjection: function (renderer, camera) {
    ...
    var translationMat = new _Matrix.default();
    translationMat.array[12] = (offset[0] * 2.0 - 1.0) / width;
    translationMat.array[13] = (offset[1] * 2.0 - 1.0) / height;

    _Matrix.default.mul(camera.projectionMatrix, translationMat, camera.projectionMatrix);

    _Matrix.default.invert(camera.invProjectionMatrix, camera.projectionMatrix);
  },
